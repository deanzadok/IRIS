...
from losses import CoverageLoss

parser = argparse.ArgumentParser()
parser.add_argument('--data_dir', '-data_dir', help='path h5 data dir', default='build/data10k', type=str)
parser.add_argument('--output_dir', '-output_dir', help='path to output folder', default='learning/test_coverageloss', type=str)
parser.add_argument('--batch_size', '-batch_size', help='number of samples in one minibatch', default=32, type=int)
parser.add_argument('--dim', '-dim', help='dimension of c space', default=5, type=int)
parser.add_argument('--num_imgs', '-num_imgs', help='number of images to train on', default=100000, type=int)
parser.add_argument('--epochs', '-epochs', help='number of epochs to train the model', default=10, type=int)
parser.add_argument('--cp_interval', '-cp_interval', help='interval for checkpoint saving', default=10, type=int)
...
def train(images, c_points):
    with tf.GradientTape() as tape:
        ...
        # compute loss
        loss = coverage_loss(predictions, predictions)
        print(loss)
    # compute step
...
train_ds, test_ds, inspection_points = load_data(data_dir=args.data_dir, num_imgs=args.num_imgs, batch_size=args.batch_size)

# create model, loss and optimizer
model = VAEModel(n_z=args.n_z)
optimizer = tf.keras.optimizers.Adam()
coverage_loss = CoverageLoss(batch_size=args.batch_size, dim=args.dim, inspection_points=inspection_points)
